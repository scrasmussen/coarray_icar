...
## down-the-line
- [ ] in domain_implementation:57 calling `this%get_grid_dimensions()` alot, one func call?
...
Use microphysics to with domain argument to call `mp_gt_driver`, which does all
  the fancy stuff
 BUT
Add to/modify the microphysics driver (`mp_driver.f90` file) to handle the
  calculations
...
domain_interface: has all the variable, initializes them,
                  calls halo send, recv, exchange
 - `domain_t`
   - `type(exchangeable_t) :: water_vapor`

exchangeable_interface: has all the halo regions, defines put and retrieve
                        subroutines
...
## notes 4.1.20
convected parcel, computationally particle if multiple points its an object
 particle fine
