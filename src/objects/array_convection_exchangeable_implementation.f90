        ...
        do j=jts,jte
          do k=kts,kte
            ...
            if (particle%exists .eqv. .true. .and. &
              ...
              T = temperature(i-dif(1), k-dif(3), j-dif(2))
              ...
              if ( .not. all( (/i,j,k/) .eq. &
                  new_ijk)) then
                if (floor(particle%z) .gt. kte ) then
                  print *, "----- SENDING PARTICLE: from",(/i,j,k/),"to",new_ijk
                  ! call particle%send_particle(new_ijk) ! probs not like this
